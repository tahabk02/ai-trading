...
export const PredictiveIntelligence: React.FC<PredictiveIntelligenceProps> = ({
  ...
  if (
    predictionData?.target_price != null &&
    Number.isFinite(predictionData.target_price) &&
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 sm:p-5 md:p-6 backdrop-blur-sm relative">
      {/* Header */}
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        <select
          ...
          onChange={(e) => { if (onTimeframeChange) onTimeframeChange(e.target.value); }}
          className="bg-gray-800 text-white text-xs sm:text-sm rounded-lg px-3 py-2 border border-gray-600 focus:outline-none focus:border-emerald-500 cursor-pointer font-mono pointer-events-auto z-50 relative"
      ...
      {error && (
        <div className="mb-3 sm:mb-4 p-2 sm:p-3 bg-rose-500/10 border border-rose-800/50 rounded-lg">
          ...
          <button onClick={() => { const s = (symbol || "").trim().toUpperCase(); if (s) getPrediction(s, selectedTimeframe); }}
            className="mt-2 text-[9px] text-blue-400 hover:text-blue-300 font-mono underline">Retry</button>
        </div>
      )}
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <span className={"px-2 sm:px-3 py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider " + (
              predictionData.signal === "BUY" ? "bg-emerald-500/20 text-emerald-400" :
              predictionData.signal === "SELL" ? "bg-rose-500/20 text-rose-400" : "bg-slate-500/20 text-slate-400"
            )}>{predictionData.signal}</span>
            ...
            {deltaPct != null && (
              <span className={"text-[10px] sm:text-xs font-bold font-mono px-2 py-0.5 rounded " + (
                Number(deltaPct) > 0 ? "text-emerald-400 bg-emerald-500/10" :
                Number(deltaPct) < 0 ? "text-rose-400 bg-rose-500/10" : "text-slate-400 bg-slate-500/10"
          ...
          <div>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-blue-500 transition-all duration-500" style={{
                width: predictionData.confidence != null ? (predictionData.confidence * 100).toFixed(1) + "%" : "0%",
              }} />
            </div>

          {/* Metrics Grid — Timeframe is STRICTLY from local selectedTimeframe prop, NEVER from API predictionData.timeframe */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
            ...
            <MetricCard label="Timeframe" value={selectedTimeframe ?? "1d"} />
            <MetricCard label="Latency" value={predictionData.proxyLatencyMs != null ? predictionData.proxyLatencyMs + "ms" : "--"} />
          </div>

          {/* Timestamp */}
          <div className="text-[9px] text-slate-600 font-mono">
            Updated: {displayTimestamp ? new Date(displayTimestamp).toLocaleString() : "--"}
          </div>
      )}
      ...
      {!predictionData && !isLoading && !error && (
        <div className="py-6 sm:py-8 text-center">
          <p className="text-slate-600 text-[11px] sm:text-xs font-mono">Enter a symbol above and click Go</p>
        </div>
      )}
    </div>
  );
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print('Written OK. Length:', len(content))
